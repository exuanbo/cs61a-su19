...
class GameTurn(object):
    ...
    def is_successor(self, other):
        ...
        if self.score0 == other.score0 and self.score1 == other.score1:
            # Originally had an extra condition for a call to is_over. However, because
            # self.rolls is mutated before we check the traces, this will cause us
            # to return False here regardless.
            # Regardless, should have a check for if a strategy function is called a second
            # time in the same roll AFTER the score is updated.
            return False
...
def make_solution_traces(hog):
    ...
    for i in range(NUM_TESTS):
        ...
        print("Created {:d}/{:d} solution traces".format(i, NUM_TESTS))
    return sol_traces
