...
from helper import pair_to_list, verify_exact_callable_length, verify_min_callable_length, make_list, dotted_pair_to_list
from lexer import TokenBuffer
from log import Holder, VisualExpression, return_symbol, logger
from scheme_exceptions import OperandDeduceError, IrreversibleOperationError, LoadError, SchemeError, TypeMismatchError, CallableResolutionError


class ProcedureObject(Callable):

    def __init__(self, params: List[Symbol], var_param: Optional[Symbol], body: List[Expression], frame: Frame, name: str = None):
        super().__init__()
        self.params = params
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder, eval_operands=True):
        ...
        if self.var_param:
            new_frame.assign(self.var_param, make_list(
                operands[len(self.params):]))
        out = None
        ...
        for (i, expression) in enumerate(body):
            out = evaluate(expression, new_frame, gui_holder.expression.children[i], (
                self.evaluates_operands and (i == (len(body) - 1))), log_stack=(len(self.body) == 1))
    ...
    def __repr__(self):
        if (self.var_param is not None):
            ...
            varparams = ''
        return ''.join(['(', '{}'.format(self.name), ' ', '{}'.format(' '.join(map(repr, self.params))), '{}'.format(varparams), ') [parent = ', '{}'.format(self.frame.id), ']'])
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder, name: str = 'lambda'):
        ...
        if ((not logger.dotted) and (not isinstance(params, (Pair, NilType)))):
            raise OperandDeduceError(''.join(
                ['Expected Pair as parameter list, received ', '{}'.format(params), '.']))
        (params, var_param) = dotted_pair_to_list(params)
        for (i, param) in enumerate(params):
            if ((logger.dotted or (i != (len(params) - 1))) and (not isinstance(param, Symbol))):
                raise OperandDeduceError(''.join(
                    ['Expected Symbol in parameter list, received ', '{}'.format(param), '.']))
            if isinstance(param, Pair):
                param_vals = pair_to_list(param)
                if ((len(param_vals) != 2) or (not isinstance(param_vals[0], Symbol)) or (not isinstance(param_vals[1], Symbol)) or (param_vals[0].value != 'variadic')):
                    raise OperandDeduceError(''.join(
                        ['Each member of a parameter list must be a Symbol or a variadic parameter, not ', '{}'.format(param), '.']))
...
class DefineMacro(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if (not isinstance(params, Pair)):
            raise OperandDeduceError(''.join(['Expected a Pair, not ', '{}'.format(
                params), ', as the first operand of define-macro.']))
        name = params.first
        operands[0] = params.rest
        if (not isinstance(name, Symbol)):
            raise OperandDeduceError(
                ''.join(['Expected a Symbol, not ', '{}'.format(name), '.']))
        frame.assign(name, Macro().execute(
            operands, frame, gui_holder, name.value))
        return name
...
class Define(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if isinstance(params, Symbol):
            ...
            frame.assign(params, evaluate(
                operands[1], frame, gui_holder.expression.children[2]))
            return params
            ...
            if (not isinstance(name, Symbol)):
                raise OperandDeduceError(
                    ''.join(['Expected a Symbol, not ', '{}'.format(name), '.']))
            frame.assign(name, Lambda().execute(
                operands, frame, gui_holder, name.value))
            return name
        else:
            raise OperandDeduceError(''.join(['Expected a Pair, not ', '{}'.format(
...
class Set(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if (not isinstance(name, Symbol)):
            raise OperandDeduceError(''.join(
                ['Expected a Symbol, not ', '{}'.format(name), ', as the first operand of set!']))
        frame.mutate(name, evaluate(
            operands[1], frame, gui_holder.expression.children[2]))
        return Undefined
...
class Apply(Applicable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder, eval_operands=True):
        ...
        if (not isinstance(func, Applicable)):
            raise OperandDeduceError(
                ''.join(['Unable to apply ', '{}'.format(func), '.']))
        gui_holder.expression.set_entries(
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        for (cond_i, cond) in enumerate(operands):
            if (not isinstance(cond, Pair)):
                raise OperandDeduceError(''.join(
                    ['Unable to evaluate clause of cond, as ', '{}'.format(cond), ' is not a Pair.']))
            ...
            if ((isinstance(expanded[0], Symbol) and (expanded[0].value == 'else')) or (eval_condition is not SingletonFalse)):
                out = eval_condition
                for (i, expr) in enumerate(expanded[1:]):
                    out = evaluate(expr, frame, cond_holder.expression.children[(
                        i + 1)], (i == (len(expanded) - 2)))
                return out
        return Undefined
...
class And(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        for (i, expr) in enumerate(operands):
            value = evaluate(expr, frame, gui_holder.expression.children[(
                i + 1)], (i == (len(operands) - 1)))
...
class Let(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if ((not isinstance(bindings, Pair)) and (bindings is not Nil)):
            raise OperandDeduceError(''.join(
                ['Expected first argument of let to be a Pair, not ', '{}'.format(bindings), '.']))
        ...
        for (i, binding) in enumerate(bindings):
            if (not isinstance(binding, Pair)):
                raise OperandDeduceError(
                    ''.join(['Expected binding to be a Pair, not ', '{}'.format(binding), '.']))
            binding_holder = bindings_holder.expression.children[i]
            binding = pair_to_list(binding)
            if (len(binding) != 2):
                raise OperandDeduceError(''.join(
                    ['Expected binding to be of length 2, not ', '{}'.format(len(binding)), '.']))
            (name, expr) = binding
            if (not isinstance(name, Symbol)):
                raise OperandDeduceError(''.join(
                    ['Expected first element of binding to be a Symbol, not ', '{}'.format(name), '.']))
            new_frame.assign(name, evaluate(
                expr, frame, binding_holder.expression.children[1]))
        value = None
    ...
    def quasiquote_evaluate(cls, expr: Expression, frame: Frame, gui_holder: Holder, splicing=False):
        ...
        if isinstance(expr, Pair):
            try:
                ...
                is_well_formed = (not any(map((lambda x: (isinstance(x, Symbol) and (
                    x.value in ['unquote', 'quasiquote', 'unquote-splicing']))), lst)))
            ...
            if (isinstance(expr.first, Symbol) and (expr.first.value in ('unquote', 'unquote-splicing'))):
                ...
                if is_well_formed:
                    ...
                    if (not logger.dotted):
                        raise OperandDeduceError(
                            ''.join(['{}'.format(expr), ' is an ill-formed quasiquotation.']))
                    out = Pair(Quasiquote.quasiquote_evaluate(expr.first, frame, visual_expression.children[0]), Quasiquote.quasiquote_evaluate(
...
class Load(Applicable):
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder, eval_operands=True):
        ...
        if (not isinstance(operands[0], Symbol)):
            raise OperandDeduceError(
                ''.join(['Load expected a Symbol, received ', '{}'.format(operands[0]), '.']))
        if logger.fragile:
            raise IrreversibleOperationError()
        try:
            with open(''.join(['{}'.format(operands[0].value), '.scm'])) as file:
                code = (('(begin-noexcept' + '\n'.join(file.readlines())) + '\n)')
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder, eval_operands=True):
        ...
        if eval_operands:
            operand = evaluate_all(
                operands, frame, gui_holder.expression.children[1:])[0]
        if (not isinstance(operand, Promise)):
            raise OperandDeduceError(
                ''.join(['Force expected a Promise, received ', '{}'.format(operand)]))
        if operand.forced:
            return operand.expr
        if logger.fragile:
            raise IrreversibleOperationError()
        ...
        if ((not logger.dotted) and (not isinstance(evaluated, (Pair, NilType)))):
            raise TypeMismatchError(''.join(['Unable to force a Promise evaluating to ', '{}'.format(
                operand.expr), ', expected another Pair or Nil']))
