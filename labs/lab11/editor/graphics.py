...
class Canvas():
    ...
    def pixel(self, x: float, y: float, color: str):
        ...
        pixel_move.seq.append(make_action(
            ABSOLUTE_MOVE, (x * self.size), (y * self.size)))
    ...
    def arc(self, signed_radius: float, degrees: float):
        ...
        def polar_to_cartesian(center_x, center_y, radius, angle_in_degrees):
            ...
            return ((center_x + (radius * math.cos(angle_in_radians))), (center_y + (radius * math.sin(angle_in_radians))))
        ...
        def draw_arc(x, y, radius, start_angle, end_angle):
            ...
            return (make_action(ABSOLUTE_ARC, radius, radius, 0, large_arc_flag, sweep_flag, end_x, end_y), end_x, end_y)
        ...
        (arc_action, end_x, end_y) = draw_arc(center_x,
                                              center_y, abs(signed_radius), degree_start, degree_end)
...
def make_color(expression: Expression) -> str:
    if ((not isinstance(expression, String)) and (not isinstance(expression, Symbol))):
        raise OperandDeduceError(''.join(
            ['Expected a String or Symbol, received ', '{}'.format(expression), '.']))
    color = expression.value.lower()
    if ((color not in COLORS) and (not re.search('^#(?:[0-9a-fA-F]{3}){1,2}$', color))):
        raise OperandDeduceError(''.join(
            ['Expected a valid CSS or hex color code, received ', '{}'.format(expression), '.']))
    return color
